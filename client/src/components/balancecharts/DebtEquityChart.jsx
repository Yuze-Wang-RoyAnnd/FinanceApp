...
export const DebtEquityChart = ({balanceInfo, h, w, update_color}) => {
  ...
  useEffect(() => {
    ...
    setCurrentRatio(balanceInfo['balance_data'].map((entry) => {
      ...
      return { key: key, v : values.map(item => {
          var de = 0
          if (item['totalEquity'] != 0) de = item['totalLiabilities'] / item['totalEquity'];
          maxValue.current = Math.max(de, maxValue.current);
          minValue.current = Math.min(de, minValue.current);
          return {'fiscalDateEnding' : item['fiscalDateEnding'], 'DERatio' : de}
      })};
    }));
  }, [balanceInfo]);
  ...
  useEffect(() => {
      ...
      const drawcurr = d3.line()
          ...
          .y(function (d) {return yScale(d['DERatio'])});
  ...
  return (
    <div className='DE' onClick={() => setDefinition(definition => !definition)}>
      {definition ? (
        <svg ref={svgRef} id='DE'></svg>
      ): (
        <div style={{display: 'flex', justifyContent:'center', alignItems:'center', textAlign: 'center', height: '100%', width: '100%'}}>
            <p style={{fontSize: '2rem', color: '#7A7A7A'}}>Debt to Equity Ratio (Total)</p>
        </div>
      )}
    </div>
  )
