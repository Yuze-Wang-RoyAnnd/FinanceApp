...
export const CashRatioChart = ({balanceInfo, h, w, update_color}) => {
    ...
    useEffect(() => {
        ...
        setCurrentRatio(balanceInfo['balance_data'].map((entry) => {
            ...
            return { key: key, v : values.map(item => {
                var cash = 0
                if (item['currentLiabilities'] != 0) cash = item['cash'] / item['currentLiabilities'];
                maxValue.current = Math.max(cash, maxValue.current);
                minValue.current = Math.min(cash, minValue.current);
                return {'fiscalDateEnding' : item['fiscalDateEnding'], 'cashRatio' : cash}
            })};
        }));
    }, [balanceInfo])

    useEffect(() => {
        d3.select("#RCASH").html("");
        ...
        const drawcurr = d3.line()
            ...
            .y(function (d) {return yScale(d['cashRatio'])});
    ...
    return (
    <div className='cashR' onClick={() => setDefinition(definition => !definition)}> 
        {definition ? (
            <svg ref={svgRef} id='RCASH'></svg>
        ) : (
            <div style={{display: 'flex', justifyContent:'center', alignItems:'center', textAlign: 'center', height: '100%', width: '100%'}}>
              <p style={{fontSize: '2rem', color: '#7A7A7A'}}>Liquidity Ratio (Cash)</p>
          </div>
        )}
